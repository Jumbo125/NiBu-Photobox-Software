...
if not exist "%TASKFILE%" (
  > "%STATUSFILE%" echo TASKNAME_FILE_MISSING
  exit /b 4
)

set /p TASKNAME=<"%TASKFILE%"
if "%TASKNAME%"=="" (
  > "%STATUSFILE%" echo TASKNAME_EMPTY
  exit /b 5
)

schtasks /query /tn "%TASKNAME%" >nul 2>&1
if not %errorlevel%==0 (
  > "%STATUSFILE%" echo TASK_NOT_FOUND
  exit /b 3
)

schtasks /run /tn "%TASKNAME%" >nul 2>&1
if not %errorlevel%==0 (
  > "%STATUSFILE%" echo START_FAILED
  exit /b 2
)
...
if /I "%TASK_STATE%"=="Running" (
  > "%STATUSFILE%" echo STARTED
  exit /b 0
)

> "%STATUSFILE%" echo START_TRIGGERED
exit /b 1
